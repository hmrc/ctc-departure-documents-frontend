...
class DeclareQuantityOfGoodsYesNoController @Inject() (
  ...
  view: DeclareQuantityOfGoodsYesNoView
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode, documentIndex: Index): Action[AnyContent] = actions.requireData(lrn) {
    implicit request =>
      val preparedForm = request.userAnswers.get(DeclareQuantityOfGoodsYesNoPage(documentIndex)) match {
        case None        => form
      ...
      form
        ...
        .fold(
          ...
          value => {
            ...
            DeclareQuantityOfGoodsYesNoPage(documentIndex).writeToUserAnswers(value).updateTask().writeToSession().navigate()
          }
        )
  }
}
