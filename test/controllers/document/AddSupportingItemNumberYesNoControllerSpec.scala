...
class AddSupportingItemNumberYesNoControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar {
  ...
  private lazy val addSupportingItemNumberYesNoRoute = routes.AddSupportingItemNumberYesNoController.onPageLoad(lrn, mode, documentIndex).url
  ...
  "AddSupportingItemNumberYesNo Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.setValue(AddSupportingItemNumberYesNoPage(documentIndex), true)
